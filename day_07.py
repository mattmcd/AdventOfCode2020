...
def read(s=None):
    ...
    edge_list = [item for sublist in records for item in sublist]

    df = pd.DataFrame(edge_list)
    G = nx.from_pandas_edgelist(df, 'src', 'dest', ['weight'], create_using=nx.DiGraph)
...
def parse(line):
    ...
    dest = [{'weight': int(b.split()[0]),
             'dest': ' '.join(b.split()[1:3])} for b in inner_bags
            if b.split()[0] != 'no']
    return [{'src': bag, 'dest': b['dest'], 'weight': b['weight']} for b in dest]
...
def n_contains(G, b):
    children = G.adj[b]
    return 0 if children == {} else sum(v['weight']*(1 + n_contains(G, k)) for k, v in children.items())


def part_01(G):
    # Direct descendents: [(n1, n2) for n1, n2 in G.edges if n2 == 'shiny gold']
    return len([n for n in G.nodes if 'shiny gold' in nx.algorithms.dag.descendants(G, n)])


def part_02(G):
    return n_contains(G, 'shiny gold')
