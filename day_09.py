...
def part_02(lst, block=25):
    target = part_01(lst, block=block)
    csum = np.cumsum(lst)
    XX, YY = np.meshgrid(csum, csum)
    pos_sum = np.argwhere(XX - YY == target)
    for s in pos_sum:
        a = min(s) + 1
        b = max(s) + 1
        if (b - a) > 1:
            return min(lst[a:b]) + max(lst[a:b])
    return pos_sum
